...
while true; do
read -p "Entrez le nom du serveur (sans espaces, ex: wazuh-server) : " SERVER_NAME
if [[ "$SERVER_NAME" =~ ^[a-zA-Z0-9_-]+$ ]]; then
break
else
echo "Nom invalide : utilisez uniquement lettres, chiffres, '-' ou '_' (pas d'espaces)."
fi
done
 
# Nettoyage d'une éventuelle installation précédente échouée
